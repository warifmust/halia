...
SYSTEM_PROMPT = (
    ...
    "2) COMPARE the taught format with what the user asked for. If they differ (different "
    "columns, different structure), STOP and ask the user before generating anything: "
    "'I see a taught format with [N columns: X, Y, Z...]. You asked for [M columns: A, B...]. "
    "Which should I use?' Never generate first and ask after. "
    "3) THEN plan — state what you need to do, which tools you'll use, in what order, and "
    "how you will follow the chosen format in your output. "
